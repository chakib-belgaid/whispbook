...
describe("generation script export", () => {
  it("captures selected chapters and edited book state", () => {
    ...
    expect(patch.chapters[0].paragraphs[0]).toEqual({
      ...
      included: true,
      voice_ranges: [{ id: "range-1", start: 0, end: 6, cast_id: "alice" }],
    });
  ...
  it("embeds portable backend payloads in the generated script", () => {
    ...
    expect(decodedConstant(script, "BOOK_PATCH_B64").cast[0].name).toBe(
      "Alice",
    );
        ...
        paragraphs: [
          {
            ...
            included: true,
            voice_ranges: [
              { id: "range-1", start: 0, end: 6, cast_id: "alice" },
            ],
          },
          {
            id: "ch-0001-p-0002",
